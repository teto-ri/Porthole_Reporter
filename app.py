...
from init_data import busan_districts, busan_districts_centers, busan_districts_data, process_types, data_categories, data_descriptions, detail_locations_base
...
def fetch_and_format_accident_data(url):
    try:
        ...
        if response.status_code == 200:
            ...
            formatted_df['district'] = formatted_df['district'].apply(lambda x: x if x in busan_districts else '기타')
            return formatted_df
        else:
...
def init_district_data():
    ...
    if 'init_data_loaded' not in st.session_state:
        ...
        EMD = gpd.read_file('LSMD_ADM_SECT_UMD_pusan\LSMD_ADM_SECT_UMD_26_202309.shp', encoding='cp949')
...
def create_map(data, district_name=None, marker=False):
    ...
    with st.spinner('Wait for it...'):
        ...
        if district_name and all(name in busan_districts_centers for name in district_name):
            center_location = get_average_center(district_name)
            m = folium.Map(location=center_location, zoom_start=13.5)  # zoom level은 적절히 조정
        elif district_name == ['기타']:
            m = folium.Map(location=[35.1379222, 129.05562775], zoom_start=9)
        else:
            m = folium.Map(location=[35.1379222, 129.05562775], zoom_start=11)
        ...
        for _, accident in data.iterrows():
            ...
            iframe = folium.IFrame(html=make_popup(accident, status_class, status_description), width=250, height=200)
